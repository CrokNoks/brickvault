...
describe('Pieces Endpoints (e2e)', () => {
  ...
  it('/api/v1/pieces (GET) - liste vide', async () => {
    const res = await request(app.getHttpServer()).get('/api/v1/pieces');
  ...
  it('/api/v1/pieces/:id (GET) - récupération', async () => {
    ...
    const res = await request(app.getHttpServer()).get(`/api/v1/pieces/${id}`);
    expect(res.status).toBe(200);
